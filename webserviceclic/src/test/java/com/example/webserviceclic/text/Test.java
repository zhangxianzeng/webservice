package com.example.webserviceclic.text;
...
public class Test {
    ...
    private static void printStudents(JaxWsDynamicClientFactory dcf) {
        Client client = dcf.createClient("http://127.0.0.1:8090/demo/StudentService?wsdl");
    ...
    private static void printUser(JaxWsDynamicClientFactory dcf) {
        Client client = dcf.createClient("http://127.0.0.1:8090/demo/UserService?wsdl");
    ...
    private static void printStudents1(JaxWsDynamicClientFactory dcf) {
        ...
        try {
            //invoke("方法名",参数1,参数2,参数3......)
            Object students = client.invoke("getAll1");
            JSONArray jsonArray = JSONArray.fromObject(students);
            System.out.println(Arrays.toString(jsonArray.toArray()));
        } catch (Exception e) {
            e.printStackTrace();
